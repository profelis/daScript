...
Heuristic best-effort inlining of plain calls and operator sites (default ON, optimized builds only): small non-generic callees splice, silently declining anything unspliceable. Host-side counterpart of ``options auto_inline_functions`` (the option overrides the policy). ``disable_auto_inline`` overrides.
