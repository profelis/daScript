...
- **DANGER — silent JIT miscompile:** a comprehension used **inline** as a `<-` move-init argument inside a struct constructor (`Foo(a <- [for (x in src); expr], b <- local)`) yields an **empty array under `-jit`** (interp is fine; a plain-local `<-` field beside it is unaffected). No diagnostic. **Hoist the comprehension to a `var` local first**, then `Foo(a <- local_comp, b <- local)`. Verified repro + detail in the [[jit-inline-comprehension-moveinit-empty]] memory
